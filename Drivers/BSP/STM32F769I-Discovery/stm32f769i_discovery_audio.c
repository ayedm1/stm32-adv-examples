...
#ifndef USBD_AUDIO_RECORD_CHANNEL_COUNT
#define USBD_AUDIO_RECORD_CHANNEL_COUNT         2
#endif

/* Buffers for right and left samples */
static int32_t                  *pScratchBuff[2*USBD_AUDIO_RECORD_CHANNEL_COUNT];
static __IO int32_t             ScratchSize;
/* Cannel number to be used: 2 channels by default */
static uint8_t                  AudioIn_ChannelNumber = USBD_AUDIO_RECORD_CHANNEL_COUNT;
...
uint8_t __IO AudioInResBit = 16;
uint8_t __IO AudioIntResByte = 2;
/**
  * @}
  */
...
/** @defgroup STM32F769I_DISCOVERY_AUDIO_Private_Function_Prototypes STM32F769I_DISCOVERY_AUDIO Private Function Prototypes
  * @{
  */
static void SAIx_Out_Init(uint32_t AudioFreq, uint8_t AudioResolution);
...
/**
  ...
  */
uint8_t BSP_AUDIO_OUT_Init(uint16_t OutputDevice, uint8_t Volume, uint32_t AudioFreq, uint8_t AudioResolution)
{
  ...
  SAIx_Out_Init(AudioFreq, AudioResolution);
...
/**
  ...
  * @retval AUDIO_OK if correct communication, else wrong communication
  */
uint8_t BSP_AUDIO_IN_Init(uint32_t AudioFreq, uint32_t BitRes, uint32_t ChnlNbr, uint16_t InputDevice)
{
  return BSP_AUDIO_IN_InitEx(InputDevice, AudioFreq, BitRes, ChnlNbr);
}
...
uint8_t BSP_AUDIO_IN_InitEx(uint16_t InputDevice, uint32_t AudioFreq, uint32_t BitRes, uint32_t ChnlNbr)
{
  ...
  else
  {
    ...
    {
      /* Initialize the codec internal registers */
      audio_drv->Init(AUDIO_I2C_ADDRESS, InputDevice, 100, AudioFreq, AudioInResBit);
    }
  }
  ...
  if (res == 2)
  {
    ...
    for(int i = 0, j = ScratchOffset; j < (ScratchOffset + sample_count); j++)
    {
      if(AudioIn_Device == INPUT_DEVICE_DIGITAL_MIC)
      {
        ...
        if(AudioIn_ChannelNumber > 2)
        {
          pbuf16[i++] = (uint16_t)(SaturaLH((pScratchBuff[3][j] >> 8), -32760, 32760));
          pbuf16[i++] = (uint16_t)(SaturaLH((pScratchBuff[2][j] >> 8), -32760, 32760));
        }
      }
      else
      {
        pbuf16[i]   = (uint16_t)pScratchBuff[0][j] ;
        pbuf16[i+1] = (uint16_t)pScratchBuff[0][j] ;
        i+=2;
      }
    }
  }
